...
● 优化策略：
  ○ 存储格式：不建议直接读原始 Wav/MP3。建议预先将音频列表及其元数据构建索引。如果是超大规模，推荐使用 WebDataset 或 Parquet 格式，将音频打包成 Shard（如 100MB 一个包），减少 OSS 请求次数。
...
### 数据层
系统主要通过阿里云OSS作为数据源，使用Parquet索引和本地缓存机制优化大规模音频数据的读取性能。数据读取流程包括文件发现、索引构建、缓存管理和预处理四个主要步骤。
...
1. 异构计算资源协同问题
  ...
  核心组件架构
   ...
   1. 串行下载：所有文件都是串行下载，没有利用并行性
   2. 重复缓存检查：每个文件都单独检查缓存，没有批量优化
   3. 临时文件开销：每个下载都创建临时文件，增加了I/O开销
   4. 内存使用：将整个文件加载到内存中，对大文件不友好